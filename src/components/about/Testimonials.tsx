      ...
      <div className="flex flex-col gap-10 max-w-[620px]">
        {testimonials.map((q, i) => (
          <blockquote key={i} className="border-l-2 border-border dark:border-ink-border pl-6">
            <p className="font-vin-pro-mono text-[15px] text-foreground dark:text-white leading-relaxed mb-4">
              &ldquo;{q.text}&rdquo;
            </p>
            <footer className="flex flex-wrap items-center gap-2">
              <span className="font-vin-pro-mono text-[13px] text-green-600 dark:text-ink-accent">
                — {q.attribution}
              </span>
              <span className="font-vin-pro-mono text-[11px] uppercase tracking-widest text-muted-foreground dark:text-ink-muted">
                {q.role} · {q.year}
              </span>
            </footer>
          </blockquote>
